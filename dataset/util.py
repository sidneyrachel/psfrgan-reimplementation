...
def convert_to_one_hot(image):
    label_num = len(MASK_COLORMAP)
    image = np.array(image, dtype=np.unit8)
    height, width = image.shape[:2]
    one_hot_label = np.zeros((label_num, height, width))
    colormap = np.array(MASK_COLORMAP).reshape(label_num, 1, 1, 3)
    colormap = np.tile(colormap, (1, height, width, 1))  # output dimension: (label_num, height, width, 3)
