...
class NormLayer(nn.Module):
    def __init__(self, num_channels, normalize_shape=None, norm_type=NormTypeEnum.BN):
        super(NormLayer, self).__init__()

        if norm_type == NormTypeEnum.BN:
            self.norm = nn.BatchNorm2d(num_channels, affine=True)
        elif norm_type == NormTypeEnum.IN:
            self.norm = nn.InstanceNorm2d(num_channels, affine=False)
        elif norm_type == NormTypeEnum.GN:
            self.norm = nn.GroupNorm(32, num_channels, affine=True)
